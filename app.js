...
let notificationPermissionTools = null;
...
async function blockPersonFromProfile(button) {
  ...
  try {
    ...
    if (chatUiState.activeContext?.withUser === username) {
...
function createDefaultNotificationPermissionState() {
  return getNotificationPermissionTools().createDefaultNotificationPermissionState();
}

function normalizeNotificationPermissionStatus(value) {
  return getNotificationPermissionTools().normalizeNotificationPermissionStatus(value);
}

function readNotificationPermissionState() {
  return getNotificationPermissionTools().readNotificationPermissionState();
}

function persistNotificationPermissionState() {
  getNotificationPermissionTools().persistNotificationPermissionState();
}

function updateNotificationPermissionState(nextState = {}) {
  return getNotificationPermissionTools().updateNotificationPermissionState(nextState);
}

function syncNotificationPermissionStateFromBrowser() {
  return getNotificationPermissionTools().syncNotificationPermissionStateFromBrowser();
}

function getNotificationPermissionState() {
  return getNotificationPermissionTools().getNotificationPermissionState();
}

function getNotificationPermissionStatusLabel(state = getNotificationPermissionState()) {
  return getNotificationPermissionTools().getNotificationPermissionStatusLabel(state);
}

function shouldShowNotificationPermissionPrompt(trigger = "", { allowDenied = false } = {}) {
  return getNotificationPermissionTools().shouldShowNotificationPermissionPrompt(trigger, { allowDenied });
}

function ensureNotificationPermissionPromptRoot() {
  return getNotificationPermissionTools().ensureNotificationPermissionPromptRoot();
}

function closeNotificationPermissionPrompt() {
  getNotificationPermissionTools().closeNotificationPermissionPrompt();
}

async function requestBrowserNotificationPermission() {
  return getNotificationPermissionTools().requestBrowserNotificationPermission();
}

function renderNotificationPermissionPromptBody(trigger = "", options = {}) {
  return getNotificationPermissionTools().renderNotificationPermissionPromptBody(trigger, options);
}

function showNotificationPermissionPrompt(trigger = "message", options = {}) {
  return getNotificationPermissionTools().showNotificationPermissionPrompt(trigger, options);
}

function openNotificationPermissionPrompt(trigger = "settings", options = {}) {
  return getNotificationPermissionTools().openNotificationPermissionPrompt(trigger, options);
}

function maybePromptNotificationPermission(trigger = "message", options = {}) {
  return getNotificationPermissionTools().maybePromptNotificationPermission(trigger, options);
}
...
function getMarketplaceVariantTools() {
  if (!marketplaceVariantTools) {
    const factory = window.WingaModules?.marketplace?.createVariantHelpers;
    marketplaceVariantTools = typeof factory === "function"
      ? factory({
        getUniqueRenderableImageIndexes,
        getRenderableMarketplaceImages
      })
      : {};
  }
  return marketplaceVariantTools;
}

function getSmartVariantIndex(feedPosition = 0, variantCount = 1) {
  return Number(getMarketplaceVariantTools().getSmartVariantIndex?.(feedPosition, variantCount) ?? 0);
}

function getFeedEntryDisplayImageIndex(item, feedPosition = 0) {
  return Number(getMarketplaceVariantTools().getFeedEntryDisplayImageIndex?.(item, feedPosition) ?? 0);
}

function getProductVariantCount(product) {
  return Number(getMarketplaceVariantTools().getProductVariantCount?.(product) ?? 1);
}

function isVariantFeedEntry(item) {
  return Boolean(getMarketplaceVariantTools().isVariantFeedEntry?.(item));
}

function buildContinuousDiscoveryCandidateKey(item) {
  return String(getMarketplaceVariantTools().buildContinuousDiscoveryCandidateKey?.(item) || "");
}

function dedupeContinuousDiscoveryFeedItems(items = []) {
  const deduped = getMarketplaceVariantTools().dedupeContinuousDiscoveryFeedItems?.(items);
  return Array.isArray(deduped) ? deduped : [];
}
...
function reorderProductImagesForVariant(product, variantIndex = 1) {
  const images = getMarketplaceVariantTools().reorderProductImagesForVariant?.(product, variantIndex);
  return Array.isArray(images) ? images : [];
}

function getVariantInitialImageIndex(product, variantIndex = 1) {
  return Number(getMarketplaceVariantTools().getVariantInitialImageIndex?.(product, variantIndex) ?? 0);
}
...
function resolvePreferredVariantImageIndex(candidateIndexes = [], preferredIndex = -1) {
  return Number(getMarketplaceVariantTools().resolvePreferredVariantImageIndex?.(candidateIndexes, preferredIndex) ?? -1);
}

function shouldUseVariantScrollSlot(normalProductOrdinal = 0) {
  return Boolean(getMarketplaceVariantTools().shouldUseVariantScrollSlot?.(normalProductOrdinal));
}

function getDeterministicVariantSlotIndex(candidateIndexes = [], options = {}) {
  return Number(getMarketplaceVariantTools().getDeterministicVariantSlotIndex?.(candidateIndexes, options) ?? -1);
}

function getStableVariantSelectionSeed(productId = "", options = {}) {
  return Number(getMarketplaceVariantTools().getStableVariantSelectionSeed?.(productId, options) ?? 0);
}
...
function getMarketplaceGalleryTools() {
  if (!marketplaceGalleryTools) {
    const factory = window.WingaModules?.marketplace?.createGalleryModule;
    marketplaceGalleryTools = typeof factory === "function"
      ? factory({
        getRenderableMarketplaceImages,
        getImageFallbackDataUri,
        isVariantFeedEntry,
        normalizeProductFitMode,
        getProductFitMode,
        preloadImageSource,
        sanitizeImageSource,
        escapeHtml,
        createProgressiveImage,
        bumpRuntimeDiagnostic,
        getPerfNow,
