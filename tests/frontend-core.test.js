...
test("home feed reserves stable media and deferred section geometry", () => {
  ...
  assert.match(gallerySource, /authorityImage\.addEventListener\("load", \(\) => \{/);
  assert.match(gallerySource, /authorityImage\.addEventListener\("load", \(\) => \{\s+syncAspectRatio\(\);\s+syncStableFeedAspectRatio\(\);/);
  assert.match(gallerySource, /if \(authorityImage\.complete[\s\S]*syncAspectRatio\(\);\s+syncStableFeedAspectRatio\(\);/);
  assert.match(gallerySource, /initSyncTimer = window\.setTimeout\(\(\) => \{[\s\S]*syncAspectRatio\(\);\s+syncStableFeedAspectRatio\(\);\s+lastTrackedIndex = syncBadge\(\);/);
  assert.doesNotMatch(gallerySource, /feedGalleryActiveRatio/);
  ...
  assert.match(appSource, /function scheduleBootPostPaintIdleWork\(lifecycleEpoch, callback, timeout = 700\) \{[\s\S]*afterNextPaint\(\(\) => \{[\s\S]*scheduleIdleBackgroundWork/);
  assert.match(appSource, /scheduleBootPostPaintIdleWork\(lifecycleEpoch, \(\) => syncNotificationPermissionStateFromBrowser\(\), 800\);/);
  assert.match(appSource, /scheduleBootAppSettingsLoad\(lifecycleEpoch\);[\s\S]*scheduleBootStartupHydration\(lifecycleEpoch\);/);
  assert.match(appSource, /function scheduleBootStartupHydration\(lifecycleEpoch\) \{[\s\S]*afterNextPaint\(\(\) => \{[\s\S]*window\.WingaDataLayer\.hydrateStartupData\(\)\.catch/);
  assert.doesNotMatch(appSource, /const appSettings = await appSettingsPromise/);
  assert.match(appSource, /function scheduleSkeletonOnlyFeedRecovery\(reason = "boot_skeleton_only_recovery", delayMs = 900\) \{[\s\S]*productsContainer\?\.querySelector\("\[data-feed-skeleton-card='true'\]"\)[\s\S]*renderCurrentView\(\{ reason, force: true \}\);/);
  assert.match(appSource, /scheduleSkeletonOnlyFeedRecovery\(`\$\{reason\}_skeleton_recovery`, 900\);[\s\S]*scheduleSkeletonOnlyFeedRecovery\(`\$\{reason\}_skeleton_recovery_late`, 2200\);/);
  assert.match(appSource, /const HOME_INFINITE_BOOT_RUNWAY_GRACE_MS = 5000;/);
  assert.match(appSource, /autoRunwayReadyAt = Number\(options\.autoRunwayReadyAt \|\| 0\) \|\| \(Date\.now\(\) \+ HOME_INFINITE_BOOT_RUNWAY_GRACE_MS\)/);
  assert.match(appSource, /function markHomeContinuationUserIntent\(\) \{[\s\S]*homeContinuousDiscoveryRuntime\.userContinuationIntentAt = Date\.now\(\);/);
  assert.match(appSource, /Date\.now\(\) < Number\(homeContinuousDiscoveryRuntime\.autoRunwayReadyAt \|\| 0\)/);
  assert.match(appSource, /!Number\(homeContinuousDiscoveryRuntime\.userContinuationIntentAt \|\| 0\)/);
  assert.match(appSource, /if \(homeContinuousDiscoveryRuntime\.backendRefreshPromise\) \{[\s\S]*if \(force\) \{[\s\S]*homeContinuousDiscoveryRuntime\.loadMoreAbortController\?\.abort\?\.\(\);[\s\S]*stalePromise\.catch\(\(\) => false\);/);
  assert.match(appSource, /if \(force\) \{[\s\S]*homeContinuousDiscoveryRuntime\.loadMoreAbortController\?\.abort\?\.\(\);[\s\S]*homeContinuousDiscoveryRuntime\.isLoadingMore = false;/);
  assert.match(appSource, /function isAbortedHomeFeedLoad\(error, signal = null\) \{[\s\S]*const status = Number\(error\?\.status \|\| 0\);[\s\S]*if \(status > 0\) \{[\s\S]*return false;/);
  assert.match(appSource, /const previousErrorWasRetryableHttp = Number\(lastError\?\.status \|\| 0\) >= 500;/);
  assert.match(appSource, /const currentErrorWasRetryableHttp = Number\(error\?\.status \|\| 0\) >= 500;/);
  assert.match(appSource, /await waitForHomeFeedRetry\(delayMs, currentErrorWasRetryableHttp \? null : signal\);/);
  assert.match(appSource, /const getLoadMoreRequestSignal = \(\) => \(abortController\?\.signal\?\.aborted \? undefined : abortController\?\.signal\);/);
  assert.match(appSource, /const committedCount = Number\(page\?\.appendedCount \|\| committedProducts\.length \|\| 0\);[\s\S]*if \(committedCount <= 0\) \{[\s\S]*return false;/);
  assert.match(appSource, /refreshPublicEntryChrome\(\{ deferHeavyChrome: true \}\);[\s\S]*const bootTargetView = getBootTargetView/);
  assert.match(appSource, /scheduleBootPostPaintIdleWork\(lifecycleEpoch, \(\) => startMemoryMonitoring\(\), 1600\);/);
...
test("global translation catalogs are versioned, validated, cached, and fail soft", () => {
  ...
  assert.match(appSource, /translateUi\("trust\.reportSubmittedTitle"/);
...
test("api writes attach a CSRF token before sending state-changing requests", () => {
  ...
  assert.match(backendSource, /function buildSelfSessionPayload\(user, options = \{\}\)/);
  ...
  assert.match(backendSource, /const SESSION_ROTATION_INTERVAL_MS = 30 \* 60 \* 1000/);
  assert.match(backendSource, /const SESSION_MAX_ACTIVE_PER_USER = 5/);
  assert.match(backendSource, /const SUSPICIOUS_LOGIN_MAX_FAILURES = 4/);
  ...
  assert.match(backendSource, /function shouldRotateSession\(session\)/);
  assert.match(backendSource, /function refreshSessionRecord\(session, req = null, options = \{\}\)/);
  assert.match(backendSource, /function sanitizeSessionForAdmin\(session = \{\}\)/);
  assert.match(backendSource, /function sanitizeSessionForSelf\(session = \{\}, currentToken = ""\)/);
  assert.match(backendSource, /function evaluateSessionRisk\(session = \{\}, req = null\)/);
  assert.match(backendSource, /function buildSessionPolicyPayload\(\)/);
  ...
  assert.match(backendSource, /externalWebhookConfigured: Boolean\(SESSION_SECURITY_NOTIFICATION_WEBHOOK_URL\)/);
  assert.match(backendSource, /url\.pathname === "\/api\/auth\/sessions"/);
  assert.match(backendSource, /auditTrail = await buildSessionAuditTrail\(user\.username/);
  assert.match(backendSource, /policy: buildSessionPolicyPayload\(\)/);
  assert.match(backendSource, /url\.pathname === "\/api\/auth\/step-up"/);
  ...
  assert.match(backendSource, /event: "session_rotated"/);
  assert.match(backendSource, /url\.pathname === "\/api\/admin\/sessions"/);
  assert.match(backendSource, /admin_session_revoked/);
  assert.match(backendSource, /session_context_step_up_required/);
  assert.match(backendSource, /dispatchSessionSecurityEvent\(buildExternalSessionSecurityEvent/);
  assert.match(backendSource, /function getSuspiciousLoginStatus\(req, identifier = "", scope = "public"\)/);
  assert.match(backendSource, /postgresStore\?\.readSuspiciousLoginAttempts/);
  assert.match(backendSource, /event: "login_throttled"/);
...
test("localized profile surfaces preserve account and commerce contracts", () => {
  ...
  assert.match(uiSource, /profile\.sellerRegistrationTitle/);
  assert.match(uiSource, /profile\.sellerUpgradeGuidance/);
  assert.match(controllerSource, /session\.revokeConfirm/);
  assert.match(controllerSource, /profile\.renderFailed/);
  assert.match(controllerSource, /data-profile-products-load-more/);
  assert.match(controllerSource, /hydrateSellerProducts/);
...
test("visual categories reuse canonical taxonomy with fail-soft localized discovery", () => {
  ...
  assert.doesNotMatch(categoryConfigSource, /visualSprite/);
  assert.equal(fs.existsSync(path.join(root, "src", "assets", "visual-categories-v2.webp")), false);
  assert.match(appSource, /function hydrateVisualCategoryPreviews\(options = \{\}\)/);
  assert.match(appSource, /queryProductsPage\.call\(window\.WingaDataLayer/);
  assert.match(appSource, /function getStableVisualCategoryIndex\(seed, length\)/);
  assert.match(appSource, /getVisualCategoryAudienceKey\(\) \+ "\\|" \+ topValue/);
  assert.match(appSource, /categorySurface: "detail"/);
  assert.match(appSource, /function returnToVisualCategoriesRoot\(\)/);
