...
test("PostgreSQL video pipeline health is durable, aggregate-only, and threshold aware", async () => {
      ...
      return {
        rows: [{
          ...
          safetyQueue: { pending: 3, processing: 1, retry: 2, submitted: 4,
            ...
            latestFailure: "stream_provider_auth_rejected:permission denied" },
  ...
  assert.deepEqual(health, {
    ...
    safetyCompleted: 18, safetyDead: 1, safetyStalled: 1,
    safetyLastFailureCode: "stream_provider_auth_rejected", oldestSafetyPendingAgeSeconds: 700.5,
...
test("public collection pages are cursor bounded and hide inaccessible collection items", async () => {
  ...
  assert.match(calls[0].text, /collection_visibility\.content_type = 'collection'/);
  assert.match(calls[0].text, /collection_visibility\.visibility/);
