...
function getCspHeader(req) {
  ...
  return [
    ...
    "object-src 'none'",
    "img-src 'self' data: blob: https://media.wingamarket.com https://wingamarket.com https://*.cloudflarestream.com https://*.videodelivery.net",
    "font-src 'self' data:",
    "media-src 'self' data: blob: https://*.cloudflarestream.com https://*.videodelivery.net",
    "script-src 'self'",
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (url.pathname.startsWith("/api/conversations/") || url.pathname.startsWith("/api/conversation-availability/")) {
      const availabilityApi = createConversationAvailabilityApi({
        collectBody, sendJson,
        findSession: (token) => findSession(store, token), readAuthToken,
        ensureMarketplaceUser: (session, targetRes) => ensureMarketplaceUser(store, session, targetRes),
        getPostgresStore: () => postgresStore
      });
      if (await availabilityApi.handle(req, res, url)) return;
    }
